...
```{r}
perc_flows_other_counties_VA <- c(mean(flows_in$r_state == 51) *100, (1 - mean(flows_in$r_state == 51)) *100)
perc_flows_other_counties_VA

agg <- flows_in[flows_in$r_state == 51, ] %>%
  group_by(r_county) %>%
...
```{r}
percent_flows_other_states <- c(mean(flows_in$r_state != 51) * 100)
...
trimmed <- agg[order(-agg$S000), ][1:10, ]
ggplot(trimmed, aes(factor(r_state), S000)) + geom_col() + coord_flip()

flows_in_other_states <- flows_in[flows_in$r_state != 51, ]

agg <- flows_in_other_states %>%
  group_by(r_county) %>%
  summarise(S000 = sum(S000))
