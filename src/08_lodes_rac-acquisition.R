...
#------------------- download LODES rac file -------------------------------- #

download.file(url = "https://lehd.ces.census.gov/data/lodes/LODES7/va/rac/va_rac_S000_JT00_2017.csv.gz",
              destfile =  "/sfs/qumulo/qhome/sp3sd/git/dspg20fairfax/data/original/va_rac_S000_JT00_2017.csv.gz")

#gunzip("/sfs/qumulo/qhome/sm9dv/dspg20fairfax/data/original/va_rac_S000_JT00_2017.csv.gz")
...
rac <- read.csv("~/git/dspg20fairfax/data/original/va_rac_S000_JT00_2017.csv",
                colClasses = c("w_geocode" = "factor"))

xwalk <- read.csv("~/git/dspg20fairfax/data/original/va_xwalk.csv",
                  colClasses = c("tabblk2010" = "factor"))

fairfax <- merge(rac, xwalk[xwalk$cty == 51059, c("tabblk2010","cty", "trct", "bgrp")], by.x = "h_geocode", by.y = "tabblk2010")
...
#write.csv(readable_names, "data/original/lodes_rac_cols.csv")

#-------------- read in readable column names and apply them to table------------------------- #
readable_col <- read.csv("~/git/dspg20fairfax/data/original/lodes_wac_cols.csv")
